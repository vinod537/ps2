...

{
...

    {
        ...
        // $user = User::where('email', $request->get('email'))->first();
        // sendMail($user, '', 'activate_account');
        // dd($user);
        // die;
        
       $NewsletterSubscriber = DB::connection('mysql2')->table('subscribers')->where('email', $request->get('email'))->first();
       

        if(blank($NewsletterSubscriber)){
          $insertGetId =   DB::connection('mysql2')->table('subscribers')->insertGetId([
                'mail_list_id'=>1,
                'uid'=> substr(str_shuffle('0123abcdefghijkl456mnopq789rstuvwxyz'),1,14),
                'email'=> $request->get('email'),
                'status'=> 'subscribed',
                'from'=> 'added',
                'ip'=>  $request->ip(),
                'subscription_type'=> 'imported',
            ...
            for ($i=1; $i < 8; $i++) { 
                if($i==1){
                    ...
                    $value = rand(11111,999999);
                }else if($i==5){ 
                    $value='1';                   
                    if(count($request->get('newsletter_type')) > 1){
                        $value = '1,2';
                    }else{
                        if (in_array('daily',$request->get('newsletter_type'))) {
                            $value = '1';
                        }elseif (in_array('weekly',$request->get('newsletter_type'))) {
                            $value = '2';
                        }
                    }
                }else if($i==6){
                ...
                if ($i==1 || $i==2 || $i==3 || $i==5) {
                    ...
                    $insertGetId1 =   DB::connection('mysql2')->table('subscriber_fields')->insertGetId($subFields);
                }
            }
            // sendMail($user, '', 'subscription_confirmation');
        ...
        if (!blank($user)) :
            ...
            $user->newsletter_type = json_encode($request->get('newsletter_type'));
            $user->save();
            // sendMail($user, '', 'already_confirmation');
        else :
            $data = [
                'email'             => $request->get('email'),
                ...
                'password'          => 123456,
                'newsletter_enable' => 1,
                'newsletter_type' => json_encode($request->get('newsletter_type')),
            ];
            $user   = Sentinel::register($data);

            $user1       = User::where('email', $user->email)->first();           
            $user1->newsletter_type = json_encode($request->get('newsletter_type'));
            $user1->save();
        ...
        return redirect()->route('home')->with('success', __('successfully_Subscribe'));
