...
Route::prefix('pharmashots')->group(function() {
     Route::post('registers', 'UserController@register');
     Route::post('forgot-password', 'UserController@forgotPassword');
     Route::post('login', 'UserController@authenticate');
     Route::post('social/login/register', 'UserController@social_register');
     ...
     Route::group(['middleware' => ['jwt.verify','loginCheck','api.localization','CheckApiKey']], function() {
         ...
         Route::prefix('user')->group(function() {
             ...
             Route::post('save-intrest', 'UserController@Usersavecategory');
             Route::post('add-to-bookmark', 'UserController@Usersavepost');
             Route::post('save-search-history', 'UserController@SaveSearchHistory');
             Route::post('remove-from-bookmark', 'UserController@UserRemovepost');
             Route::get('bookmark-list', 'UserController@UserAllpost');
             Route::get('search-history-list', 'UserController@SearchHistoryList');
             Route::get('/discover-user-intrest-posts', 'CategoryController@discoverUserIntrestPosts');
             Route::get('/daily-news', 'CategoryController@DailyNews');
         });
    ...
    Route::group(['middleware' => ['api.localization', 'CheckApiKey']], function() {
        ...
        Route::get('/post-by-company/{id}/{slug}', 'PostController@postByCompany');
        Route::get('/post-by-product/{id}/{slug}', 'PostController@postByProduct');
        ...
        //post details url
        Route::get('/detail/{id}', 'PostController@articleDetail')->name('article.api.detail');
        Route::get('/show/{id}/{slug}', 'PostController@show')->name('article.show');
        ...
        Route::get('/category-with-post', 'CategoryController@CategoryWithPost');
