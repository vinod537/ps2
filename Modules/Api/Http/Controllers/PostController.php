...
class PostController extends Controller
{
    ...
    public function searchPost(Request $request){
        ...
        $posts = Post::with('image','video','category:id,category_name','user:id,first_name,last_name,email')
            ...
            ->select('id','old_id','press_release_link','category_id','image_id','user_id','title','title2','slug','slug2','content','content2','daily_news','post_type','tags','product2','company_name2','created_at')
    ...
    public function postDetails($slug){
        ...
        $related_post = Post::where('category_id', $categoryId)
            ...
            ->where('status',1)
            ->where('language',$language)
            ->select('id','category_id','press_release_link','user_id','image_id','video_id','title','slug','content','created_at')
            ->orderBy('created_at','DESC')
            ->take(8)
            ->get();
    ...
    public function articleDetail(Request $request, $id){
        ...
        $post = Post::with(['image:id,og_image','video','category:id,category_name,is_featured','user:id,first_name,last_name,email,social_media,profile_image','audio'])
            ->Where('id', $id)
            ->where('status',1)
            ->select('id','category_id','press_release_link','language','user_id','image_id','post_type','video_id','video_url_type','video_url','video_thumbnail_id','title','slug','content','contents','tags','created_at')
            ->first();
        ...
        $post['url'] = route('article.detail', ['id' => $post['id']]);
    ...
    public function postByCompany(Request $request, $numerofpost, $companyName){

        $language   = $request->lang ?? settingHelper('default_language');

        $ComDetail = Company::where('company_name', $companyName)->first();
        if ($ComDetail) {
            $slugs = $ComDetail->slug;
        }else{
            $slugs = '21500000000000';
        }
        $posts = Post::with('image','user:id,first_name,last_name')
            ->where('visibility', 1)
            ->where('status', 1)
            ->select('id','image_id','user_id','title','category_id','slug','content','post_type','tags','company_name2','product2','created_at')
            ->where('company_name2', 'like', '%"' . $slugs . '"%')
            ->orderBy('id', 'desc')
            ->limit($numerofpost)
            ->get();
            ...
            if(count($posts) > 0){
                $posts =    $this->imageUrlset($posts);
                  $posts =  $this->AllPostsWithCategoriesDetailsDealForma($posts);
            }
    ...
    public function postByProduct(Request $request, $numerofpost, $ProductName){

        $language   = $request->lang ?? settingHelper('default_language');

        $ComDetail = Product::where('product_name', $ProductName)->first();
        if ($ComDetail) {
            $slugs = $ComDetail->slug;
        }else{
            $slugs = '21500000000000';
    ...
    public function postBySubCategory(Request $request, $id){
        ...
        $posts = Post::with('image','user:id,first_name,last_name')
            ...
            ->select('id','category_id','press_release_link','sub_category_id','image_id','user_id','title','slug','post_type','created_at')
