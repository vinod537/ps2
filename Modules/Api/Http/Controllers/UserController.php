...
class UserController extends Controller
{
    ...
    {   
        // DB::table('apistore')->insert([
        //     'datas' => json_encode($_REQUEST),
        // ]);

        $request['first_name'] = $_REQUEST['first_name'];
        $request['last_name'] = $_REQUEST['last_name'];
        $request['email'] = $_REQUEST['email'];
        $request['social_type'] = $_REQUEST['social_type'];
        $request['unique_id'] = $_REQUEST['unique_id'];
        $request['password'] = $_REQUEST['unique_id'];
        $request['password_confirmation'] = $_REQUEST['unique_id'];
        ...
        try{
            ...
            if ($request->social_type == 'google') {
                $userFinal = User::where('email', $request->email)
                ->where('social_type', 'linkedin')
                ->first();

                if ($userFinal) {
                        return $this->responseWithError(__('You have already signed up through Linkedin.'), [], 410);
                    }
            }else{         
                ...
                if ($userFinal) {
                    return $this->responseWithError(__('You have already signed up through Google.'), [], 410);
            ...
            $user['token'] = $token;
            return $this->responseWithSuccess(__('successfully_registered'), $user, 200);
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }
    ...
    {         
        ...
        try{
            ...
            $credentials = $request->only('email', 'social_type','unique_id');
            ...
            $token = JWTAuth::fromUser($userFinal);  
    ...
    //         }else{               
    //             $user->phone   = rand(100000000000000,9999999999999);
    //         }
    ...
    public function Usersavecategory(Request $request)
    {
        try{
            ...
            $user->user_intrest = json_encode(explode(',',$request->category));            
            $user->save();          
    ...
    public function Usersavepost(Request $request)
    {
        try{
            ...
            $userBookmark = [];
            if($user->user_intrest_bookmark != null && $user->user_intrest_bookmark != '') {                 
                if(in_array($request->post_id, json_decode($user->user_intrest_bookmark))){                   
                    return $this->responseWithError(__('post_already_exist_in_your_list'), [], 500);
                }else{     
                    $userBookmark = json_decode($user->user_intrest_bookmark);           
                    array_push($userBookmark, $request->post_id);                    
                    $user->user_intrest_bookmark = $userBookmark;            
                    $user->save();    
                }
            }else{
                    array_push($userBookmark, $request->post_id);                    
            ...
            return $this->responseWithSuccess(__('successfully_added'), '', 200);
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }
    ...
    public function SaveSearchHistory(Request $request)
    {
        try{
            ...
            $userBookmark = [];
            if($user->user_save_search_history != null && $user->user_save_search_history != '') {                 
                if(in_array($request->search_content, json_decode($user->user_save_search_history))){
                    $userBookmark = json_decode($user->user_save_search_history);
                    $userBookmark = array_merge(array_diff($userBookmark, array($request->search_content)));

                    if(count($userBookmark) >= 6){
                        array_shift($userBookmark);
                    } 
                             
                    array_push($userBookmark, $request->search_content);                    
                    $user->user_save_search_history = $userBookmark;            
                    $user->save(); 
                }else{     
                    $userBookmark = json_decode($user->user_save_search_history); 
    ...
    public function UserRemovepost(Request $request)
    {
        try{
            ...
            if($user->user_intrest_bookmark != null && $user->user_intrest_bookmark != '') {                 
                if(in_array($request->post_id, json_decode($user->user_intrest_bookmark))){
                    ...
                    $userBookmark = array_merge(array_diff($userBookmark, array($request->post_id)));
                
                    $user->user_intrest_bookmark = $userBookmark;            
                    $user->save();

                }else{   
                    return $this->responseWithError(__('not_found'), [], 500);
                }
            }

            return $this->responseWithSuccess(__('successfully_remove'), '', 200);
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }

    public function UserAllpost(Request $request)
    {        
        try{           
            ...
            if($user->user_intrest_bookmark != null && $user->user_intrest_bookmark != '') { 
                ...
                if(count(json_decode($user->user_intrest_bookmark)) > 0) {
                ...
                $posts = Post::with('image','video','category:id,category_name','user:id,first_name,last_name,email')
                    ...
                    ->select('id','old_id','press_release_link','category_id','image_id','user_id','title','title2','slug','slug2','content','content2','daily_news','post_type','tags','product','company_name','created_at')
                    ->orderBy('created_at')
                    ->paginate(100);

                    $posts =    $this->imageUrlset($this->commentsCount($this->dateToHuman($posts->items())));
    ...
    {        
        try{           
            ...
            if($user->user_save_search_history != null && $user->user_save_search_history != '') { 
                ...
                if(count(json_decode($user->user_save_search_history)) > 0) {
            ...
            return $this->responseWithSuccess(__('successfully_found'), $userBookmark, 200);                
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }
    ...
    public function deleteAccount(Request $request)
    {
        ...
        //    dd($user->id);
            ...
            Sentinel::logout();
            return $this->responseWithSuccess(__('successfully_deleted'), [], 200);
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }
...
//                $request['password'] = 'social-login';
                ...
                $this->activation($user->email, $activation->code);
