...

{
    ...
    public function saveNewCategory(Request $request){
        ...
        $category->image_id_api         = $request->image_id_api;
...
//        $category->block_style          = $request->block_style;
...

    {
        ...
        $activeLang     = Language::where('status', 'active')->orderBy('name', 'ASC')->get();
        $imagethumbnail = '';
        if ($category->image_id && $category->image_id !='' && $category->image_id != null) {
            $image          = galleryImage::find($category->image_id);            
            if ($image ) {
        ...
        $imagethumbnailapi = '';
        if ($category->image_id_api && $category->image_id_api !='' && $category->image_id_api != null) {
            $image          = galleryImage::find($category->image_id_api);            
            if ($image ) {
        ...
        return view('post::edit_category', compact('category', 'activeLang', 'imagethumbnail','imagethumbnailapi'));
