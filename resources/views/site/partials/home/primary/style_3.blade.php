...
.img__08 img {
  ...
  .hdaily_s h3 {
   ...
   background:url({{static_asset('site/images/back.jpg') }});
   }
         ...
         <div class="col-lg-12">
            <div id="home-carouselgfgfgfg" class=" ">
               <div class="owl-carousel">
                  @foreach($slider1 as $postslider1s)
                  <div class="item">
   				   <a href="{{$postslider1s->url}}">
   					@if(isFileExist(@$postslider1s->adImage, $result = @$postslider1s->adImage->big_image))
   					<img src="{{basePath($postslider1s->adImage)}}/{{ $result }}" data-src="{{basePath($postslider1s->adImage)}}/{{ $result }}">
   					@else
...
<div class="leftads">     
				@foreach($ads_AdLocation as $key => $ads_AdLocations)
					@if($ads_AdLocations->unique_name == 'home_page_left')
            ...
            <div class="rightads">
				@foreach($ads_AdLocation as $key => $ads_AdLocations)
					@if($ads_AdLocations->unique_name == 'home_page_right')
						@if($ads_AdLocations->ad->ad_type == 'image')
						<a href="{{$ads_AdLocations->ad->ad_url}}">
								<img src=" {{static_asset($ads_AdLocations->ad->adImage->original_image)}}" alt="" style="">	
							</a>
   ...
   <div class="topads">
            ...
            @foreach($ads_AdLocation as $key => $ads_AdLocations)
                @if($ads_AdLocations->unique_name == 'home_page_top')
                    @if($ads_AdLocations->ad->ad_type == 'image')
                        <a href="{{$ads_AdLocations->ad->ad_url}}">
                            <img src=" {{static_asset($ads_AdLocations->ad->adImage->original_image)}}" alt="" style="">	
                        </a>
                        ...
                        <div class="col-md-6">
                           <div class="l__contents1 newhead_new">
                              <h1 style="color: #ef3a0b;
                                 font-size: 60px;
                                 margin-bottom: 50px;">{{$postslider1s->title}} </h1>

<!-- {{$postslider1s->content}} -->
                            {!! @$postslider1s->content !!}
                                  <!-- @if($i == 4)
                                       <div class="s_dd">
                         <ul class="cont__yr">
                              ...
                              <div class="sg-sidebar__theiaStickySidebar">
                                  @if($postslider1s->button_type == 'Newsletter')
                                        <!-- <div class="tr-form homenewsletter">
                                            <label for="news" class="d-none">Newsletter</label>
                                            <input name="email" id="homenews" type="email" class="form-control" placeholder="{{__('email_address')}}" required>
								    ...
								    @elseif ($postslider1s->button_type == 'MobileApps')
                                        <div class="mainclasssliderimg">
                                            <ul>
											<li><a href="https://play.google.com/store/apps/details?id=com.pharmashots"> <img src="{{static_asset('site/images/')}}/gp.png"></a></li>
                                    ...
                                    @else
                                        @if($postslider1s->url1 && $postslider1s->content1)
                                        <div class="mainclasssliderimg">
                                             <a href="{{$postslider1s->url1}}" class="btn btn-success ">{{$postslider1s->content1}}</a>
                                         </div>

                                        @endif

                                        @if($postslider1s->url2 && $postslider1s->content2)
                                        <div class="mainclasssliderimg">
                                            <a href="{{$postslider1s->url2}}" class="btn btn-success ">{{$postslider1s->content2}}</a>
                                         </div>
                                        @endif
                        ...
                        @if (!isMobile())
                            <div class="col-md-6">
                                <div class="img__banners">  
                                    @if (isFileExist(@$postslider1s->adImage, $result = @$postslider1s->adImage->original_image))
                                        <img src="{{basePath($postslider1s->adImage)}}/{{ $result }}" data-src="{{basePath($postslider1s->adImage)}}/{{ $result }}">
                                    @else
                                        <img src="{{static_asset('default-image/default-100x100.png') }} "  >
                                    @endif
   ...
   <div class="container"> 
      <div class="s__09d_0 exccinte new_exccinte_2">
         ...
         <div class="item">
            ...
            @endif
            @if(isFileExist(@$interviews->ad->adImage, $result = @$interviews->ad->adImage->original_image))
               <a href="{{$interviews->ad->ad_url}}">
                   <img src="{{basePath($interviews->ad->adImage)}}/{{ $result }}" data-src="{{basePath($interviews->ad->adImage)}}/{{ $result }}">
                </a>
            @else
            <img src="{{static_asset('default-image/default-100x100.png') }} "  >
            @endif					
...
@if(count($interviewsAdsMobile) > 0)
<div class="add__banners">    
       
   <div class="container"> 
      <div class="s__09d_0 exccinte new_exccinte">
         @foreach($interviewsAdsMobile as $interviews)
         <?php 
            // dd($interviews->ad) 
            ?>
...
@if(count($interviewsAds2) > 0 || count($interviewsAdsMobile2) > 0)
    <h3 class="exclusiveintheading">EXCLUSIVE INTERVIEW</h3>
@endif

@if(count($interviewsAds2) > 0)
<div class="add__banners">    	 
   <div class="container"> 
      <div class="s__09d_0 exccinte new_excintte23">
         @foreach($interviewsAds2 as $interviews)       
         <div class="item">
            @if(isFileExist(@$interviews->ad->adImage, $result = @$interviews->ad->adImage->original_image))
               <a href="{{$interviews->ad->ad_url}}">
                   <img src="{{basePath($interviews->ad->adImage)}}/{{ $result }}" data-src="{{basePath($interviews->ad->adImage)}}/{{ $result }}">
...
@if(count($interviewsAdsMobile2) > 0)
<div class="add__banners">    	 
   <div class="container"> 
      <div class="s__09d_0 exccinte">
         @foreach($interviewsAdsMobile2 as $interviews)
         <?php 
            // dd($interviews->ad) 
            ?>
         <div class="mbile_viewimg">
...
<!--         <p style="padding-bottom: 26px;">Lorem Ipsum is simply dummy text of the printing and typesetting</p>-->
      ...
      <div class="row">
         @foreach($dailynews as $post)
         <div class="col-md-4">
            <div class="daily__news__pro">
               <div class="daily_img">				  
                  @if(isFileExist(@$post->image, @$post->image->original_image))
                  <img class="img-fluid"
                     src="{{basePath(@$post->image)}}/{{ $post->image->original_image }}"
                     data-original="{{basePath(@$post->image)}}/{{ $post->image->original_image }}"
                     alt="{!! $post->alt_tag !!}">
                  @else
                  <img src="{{static_asset('default-image/default-358x215.png') }} "
                     class="img-fluid" alt="{!! $post->alt_tag !!}">
                  @endif
               </div>
               <div class="hdaily_s">
                  <h3><a href="{{ route('article.detail', ['slug' => $post->slug, 'id' => $post->id]) }}">{!! \Illuminate\Support\Str::limit($post->title,130) !!}</a></h3>
      ...
      <p class="view__all__09"><span><a style="color: #fff;" href="{{ url('/newshome') }}">Show More</a></span></p>
...
<div class="row">
	...
	<div class="app__mobile__img">
		<img src="{{static_asset('pmobile.png') }} "  >
		</div>
	</div>
	...
	<div class="new__con__app6755">
		...
		<ul>
			<li><a href="https://play.google.com/store/apps/details?id=com.pharmashots" target="_blank"><img src="{{static_asset('play1.svg') }} "></a></li>
			<li><a href="https://apps.apple.com/in/app/pharma-shots/id1526497705/" target="_blank"><img src="{{static_asset('play2.svg') }} "></a></li>
...
{{--                   
                       <h6 class="widget-title">{{ __('Daily Snapshot(Daily Alerts)') }}</h6>

                
                    <div class="tr-form homenewsletter1">
                        <label for="news" class="d-none">Newsletter</label>
                        <input name="email" id="homenews1" type="email" class="form-control" placeholder="{{__('email_address')}}" required>
		   ...
		   {{-- <div class="col-md-6">
                <div class="sg-widget news__0f1245">
                    ...
                    <div class="widget-newsletter text-center">
                        ...
                        <h6 class="widget-title">{{ __('Weekly Snapshot(Weekly Alert)') }}</h6>

                        <!-- <p style="text-transform: uppercase;
                                margin-top: -19px;
                                margin-bottom: 24px;">{{ __('Join the PharmaShots Family') }}</p> -->
                        ...
                        <!-- <form action="{{ route('subscribe.newsletter') }}" class="tr-form" method="POST">
                            ...
                            <button class="b__9656" type="submit">Subscribe Now<span class="d-none">{{__('email_address')}}</span></button>
                        </form> -->
      ...
      <div class="row">
         ...
         <div class="col-md-{{$class}}">
            <div class="video_sections__pro">
                <iframe width="100%" height="500" data-src="{{$youtubevideo->embed_video}}?controls=0"></iframe>
            </div>  
         </div>
         @endforeach
      </div>      	
                  ...
                  <div class="list__categories__li_sting">
                     <a href="{{ url('category') }}/{{$category->slug}} ">
                        <div class="img__08 ">
                           ...
                           @if(isFileExist(@$category->image, $result = @$category->image->thumbnail))
                           <img class="" src="{{basePath($category->image)}}/{{ $result }}" data-src="{{basePath($category->image)}}/{{ $result }}">
                           @else
                           <img src="{{static_asset('default-100x100.png') }} "  >
                           @endif		
                        </div>
                        <div class="static_09">
...
<div class="modal fade" id="newsletterpopup" role="dialog" aria-modal="true" >
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('subscribe1.newsletter') }}" class="tr-form" method="POST">
            @csrf
            ...
            <input type="checkbox" required value="">I agree to the privacy policy and terms.(<a href="{{url('/')}}/app-privacy-policy">Link</a>)
            </label>
                    <div class="submit_button">

            <input name="email" id="popupemail" type="hidden" class="form-control" placeholder="{{__('email_address')}}" required>
