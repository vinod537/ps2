            ...
            @if(data_get($ad, 'ad_type') == 'image')
                <a href="{{ data_get($ad, 'ad_url', '#') ?  data_get($ad, 'ad_url', '#') : '#'}}">
                    @if(isFileExist(@$ad->adImage, $result = @$ad->adImage->original_image))
                        <img src="{{ safari_check() ? basePath(@$ad->adImage).'/'.$result : static_asset('default-image/default-add-350x350.png') }} " data-original="{{basePath($ad->adImage)}}/{{ $result }}" class="img-fluid"   alt="{!! $ad->ad_name !!}"  >
                    @else
